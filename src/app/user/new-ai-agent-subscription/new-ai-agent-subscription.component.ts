...
// }
  ...
  navigateToPurchaseAgent(plan){
    ...
    if(plan.name =="RFP"){
      this.router.navigate(['/subscription/rfp'], { queryParams: { token: this.emailToken, id:plan.id, isYearly:plan.selectedTire =='Monthly'?'false':'true' } });
      return;
      }
    if(plan.name =="Developer Agent"){
      this.router.navigate(['/subscription/dev'], { queryParams: { token: this.emailToken, id:plan.id, isYearly:plan.selectedTire =='Monthly'?'false':'true' } });
      return;
    }
    ...
    if(plan.name =="Customer Support"){
      this.router.navigate(['/subscription/chatbot'], { queryParams: { token: this.emailToken, id:plan.id, isYearly:plan.selectedTire =='Monthly'?'false':'true' } });
      return;
    }
